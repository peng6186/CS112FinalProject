...
var upstream chan int64 = make(chan int64, 20)
var downstream chan int64 = make(chan int64, 20)
...
func main() {
	...
	/* launch three daemon go routines */
	go updateDownStream()
	go updateUpstream()
	go calThroughput()
...
func relayInfo(src net.Conn, dst net.Conn) {
	...
	for {
		...
		if n > 0 {
			log.Printf("[Relay Downstreaxm: Num copied ]The n is %v\n:", n)
...
func calThroughput() {
	var down_data_t0, down_data_t1, up_data_t0, up_data_t1, diff_down, diff_up int64
	down_data_t0 = Data_Downstream
	up_data_t0 = Data_Upstream
	for {

		time.Sleep(5 * time.Second) // cal every 1 sec
		down_data_t1 = Data_Downstream
		up_data_t1 = Data_Upstream
		/* calc downstream speed and accumulated stats */
		diff_down = down_data_t1 - down_data_t0
		down_data_t0 = down_data_t1
		diff_up = up_data_t1 - up_data_t0
		up_data_t0 = up_data_t1
		log.Printf("[Speed: Downstream] The recent downstream speed is %.2f kb/s\n", float64(diff_down)/1024)
		log.Printf("[Cumulative Data: DownStream]:  %v bytes\n", Data_Downstream)
		/* calc upstream speed and accumulated stats */

		log.Printf("[Speed: Upstream] The recent upstream speed is %.2f kb/s\n", float64(diff_up)/1024)
